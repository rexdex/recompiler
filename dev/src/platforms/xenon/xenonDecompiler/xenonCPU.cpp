...
public:
	...
	virtual const bool Decompile(class ILogOutput& output, const uint32 address, const decoding::Instruction& op, const decoding::InstructionExtendedInfo& exInfo, class code::Generator& codeGen, std::string& outCode) const
	{
		...
		if (flags == 0x1F)
		{
			...
			if (!codeGen.AddInterruptCall(0, index, symbolName, symbolSignature))
				return false;

			PrintCodef( outCode, "%s;\n%s(0x%08X, regs);", 
				symbolSignature.c_str(),
				symbolName.c_str(),
				address );
...
public:
	...
	virtual const bool Decompile(class ILogOutput& output, const uint32 address, const decoding::Instruction& op, const decoding::InstructionExtendedInfo& exInfo, class code::Generator& codeGen, std::string& outCode) const
	{
		...
		if (flags == 0x1F)
		{
			std::string symbolName;
			std::string symbolSignature;
			if (!codeGen.AddInterruptCall(0, 0xFF, symbolName, symbolSignature))
				return false;
