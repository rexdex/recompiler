...
#include "../../../framework/backend/decodingContext.h"
#include "../../../framework/backend/image.h"
#include "../../../framework/backend/decodingMemoryMap.h"
#include "../../../framework/backend/decodingNameMap.h"
#include "../../../framework/backend/codeGenerator.h"
...
{
	CScopedTask task(log, "Creating segments...");

	// scan the code sections
	const image::Binary* image = decodingContext.GetImage();
	...
	for ( uint32 i=0; i<numSections; ++i )
	{
		...
		// get section range
		const uint32 start = section->GetVirtualAddress() + image->GetBaseAddress();
		const uint32 end = start + section->GetVirtualSize();
		log.SetTaskName( "Creating code segments in '%s'...", section->GetName() );
		...
		while (cur < end)
		{
			...
			log.SetTaskProgress(cur-start, end-start);
...
const bool CodeGeneratorXenon::AddBlock(class ILogOutput& log, const uint32 start, const uint32 end, class code::Generator& codeGen)
{
	// start block
...
const bool CodeGeneratorXenon::Emit(class ILogOutput& log, class code::Generator& codeGen) const
{
	// empty block
...
bool DecompilationXenon::ExportCode( ILogOutput& log, decoding::Context& decodingContext, class code::Generator& codeGen ) const
{
	// parse options
	const bool withDebugging = (codeGen.GetCompilationMode() == decoding::CompilationMode::Debug);
	CodeGeneratorOptionsXenon options(withDebugging);

	// emit the image
	if (!codeGen.AddImageData( log, decodingContext.GetImage()->GetMemory(), decodingContext.GetImage()->GetMemorySize(), decodingContext.GetBaseOffsetRVA() ))
	{
		log.Error( "Compile: Failed to export image data" );
		return false;
	}
	...
	codeGen.AddInclude(L"xenonCPU.h");
	codeGen.AddInclude(L"xenonCPU.h");
...
bool decoding::InterfaceXenon::GenerateBranchCode( ILogOutput& log, const decoding::CodeExportOptions& options, class code::Generator& codeGen, const uint32 address, const uint32 branchType, const char* ctrRegName, const char* branchCode, const bool simple )
{
	/*
	if (64-bit mode)
	then M  0
...
bool decoding::InterfaceXenon::ExportCodeInstructions( ILogOutput& log, const decoding::CodeExportOptions& options, class code::Generator& codeGen )
{
	log.SetTaskName( "Generating code..." );
...
bool decoding::InterfaceXenon::ExportSingleInstruction( ILogOutput& log, const decoding::CodeExportOptions& options, class code::Generator& codeGen, const uint32 address, const decoding::Instruction& op )
{	
