...
#include "../../../framework/backend/decodingAddressMap.h"
#include "../../../framework/backend/decodingMemoryMap.h"
#include "../../../framework/backend/decodingNameMap.h"
#include "../../../framework/backend/decodingCommentMap.h"
...
public:
	...
	const image::Section* GetSectionForAddress(const uint32 targetAddress) const
	{
		const uint32 baseAddress = m_context->GetImage()->GetBaseAddress();
		const uint32 numSections = m_context->GetImage()->GetNumSections();
		for (uint32 i=0; i<numSections; ++i)
		{
			const image::Section* section = m_context->GetImage()->GetSection(i);
			const uint32 sectionStartAddress = section->GetVirtualAddress() + baseAddress;
			const uint32 sectionEndAddress = sectionStartAddress + section->GetVirtualSize();
...
{
	const image::Binary* image = context.GetImage();
	...
	// apply known symbols
	{
		...
		for ( uint32 i=0; i<numImports; ++i )
		{
			...
			{
				const uint32 entryAddress = import->GetEntryAddress();
			...
			{
				const uint32 tableAddress = import->GetTableAddress();
	...
	bool returnStatus = true;
	{
		...
		for ( uint32 i=0; i<numSections; ++i )
		{
			...
			log.Log( "Decode: Code section start: %06Xh", section->GetVirtualAddress() );
			log.Log( "Decode: Code section size: %06Xh", section->GetVirtualSize() );

			// compute code range
			const uint32 baseAddress = image->GetBaseAddress();
			const uint32 sectionBaseAddress = baseAddress + section->GetVirtualAddress();
			const uint32 endAddress = baseAddress + section->GetVirtualAddress() + section->GetVirtualSize();
	...
	if (returnStatus)
	{
		// decode indirect addresses
		{
			...
			for ( uint32 i=0; i<numSections; ++i )
			{
				...
				// start decoding phase
				log.SetTaskName( "Resolving addresses in section '%s'...", section->GetName() );
		...
		// pointed code blocks
		{
			...
			for ( uint32 i=0; i<numSections; ++i )
			{
				...
				// start decoding phase
				log.SetTaskName( "Resolving code pointers in section '%s'...", section->GetName() );
				...
				while (address < endAddress)
				{
					...
					if ( targetAddress >= image->GetBaseAddress() && targetAddress < (image->GetBaseAddress() + image->GetMemorySize()) )
					{
						...
						for ( uint32 j=0; j<numSections; ++j )
						{
							...
							if ( targetAddress >= (baseAddress + testSection->GetVirtualAddress()) &&
								targetAddress < (baseAddress + testSection->GetVirtualAddress() + testSection->GetVirtualSize() ) )
							{
						...
						if ( targetSection )
						{
							...
							if ( targetSection->CanExecute() )
							{
								...
								if ( targetAddress & 3 )
								{
									log.Log( "Decode: Found non aligned reference to code at address %08Xh, section '%s'", address, section->GetName() );
								}
								else
								{							
							...
							else
							{
								...
								//log.Log( "Decode: Found reference to data at address %08Xh, section '%s'", address, section->GetName() );
								numDataPointers += 1;
							}
						}
					}
	...
	if (returnStatus)
	{
		...
		for ( uint32 i=0; i<numSections; ++i )
		{
			...
			// start decoding phase
			log.SetTaskName( "Resolving mapped memory in section '%s'...", section->GetName() );
