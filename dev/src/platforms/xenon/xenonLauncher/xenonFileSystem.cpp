...
#include "..\..\..\launcher\backend\launcherUtils.h"
...
namespace xenon
{
		...
		: m_kernel(kernel)
	{
		...
		if (commandline.HasOption("dvd"))
		{
			...
			const auto nativePath = commandline.GetOptionValueW("dvd");
			Mount(new FileSystemDevice_PathRedirection(kernel, nativeFileSystem, "\\Device\\Cdrom0", "DVD", nativePath));
		}
		else if (commandline.HasOption("stfs"))
		{
			Link("game:", "\\Device\\Cdrom0");
			Link("d:", "\\Device\\Cdrom0");

			const auto nativePath = commandline.GetOptionValueW("stfs");
			Mount(new FileSystemDevice_PathRedirection(kernel, nativeFileSystem, "\\Device\\Cdrom0", "STFS", nativePath));
		}
		else if (commandline.HasOption("host"))
		{
			Link("game:", "\\Device\\Harddisk1\\Partition0");
			Link("d:", "\\Device\\Harddisk1\\Partition0");

			const auto nativePath = commandline.GetOptionValueW("host");
			Mount(new FileSystemDevice_PathRedirection(kernel, nativeFileSystem, "\\Device\\Harddisk1\\Partition0", "Host", nativePath));
		}

		// dev path
		if (commandline.HasOption("devkit"))
		{
			Link("e:", "\\Device\\Harddisk1\\Partition1");
			Link("devkit:", "\\Device\\Harddisk1\\Partition1");

			const auto nativePath = commandline.GetOptionValueW("devkit");
			Mount(new FileSystemDevice_PathRedirection(kernel, nativeFileSystem, "\\Device\\Harddisk1\\Partition1", "DevKit", nativePath));
		}
	}
