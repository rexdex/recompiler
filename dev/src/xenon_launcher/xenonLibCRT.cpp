...
namespace xenon
{
	namespace lib
	{
		...
		{
			template<typename T>
			static inline void Append(T& stream, T streamEnd, char value)
			{
				*stream = value;
			...
			static inline void AppendString(T& stream, T streamEnd, const W* value)
			{
				while (*value)
				{
			...
			template<typename T>
			static inline void AppendInt(T& stream, T streamEnd, const uint64 value)
			{
				char buf[64];
			...
			template<typename T>
			static inline void AppendUint(T& stream, T streamEnd, const uint64 value)
			{
				char buf[64];
				uint32 u32 = (uint32)value;
				sprintf_s(buf, "%u", u32);
		...
		{
			const auto targetBuf = Pointer<char>(regs.R3);
			const auto targetBufSize = (uint32)regs.R4;
			const auto formatStr = Pointer<char>(regs.R5);

			{
				...
				// output string
				auto write = targetBuf;
				const auto writeEnd = targetBuf + targetBufSize-1;
				...
				while ( cur < end )
				{
					...
					if (ch == '%')
					{
						...
						switch ( code )
						{
							...
							case 'X':
							{
								...
								helper::AppendHex(write, writeEnd, val);
								++curReg;
								break;
							}
						}
					...
					{
						helper::Append(write,writeEnd,ch);
					}
				}
				...
				// end of string
				helper::Append(write, writeEnd,0);
			}
		...
		{
			const auto targetBuf = Pointer<char>(regs.R3);
			const auto targetBufSize = 1024;
			const auto formatStr = Pointer<char>(regs.R4);
		...
		uint64 __fastcall Xbox_RtlUnicodeToMultiByteN( uint64 ip, cpu::CpuRegs& regs )
		{
			const auto targetBuf = Pointer<char>(regs.R3);
			const auto targetLength = (uint32)regs.R4;
			const auto writtenPtr = Pointer<uint32>(regs.R5);
			const auto sourcePtr = Pointer<wchar_t>(regs.R6);
			const auto sourceLengthBytes = (uint32)regs.R7;
		...
		uint64 __fastcall Xbox_RtlMultiByteToUnicodeN( uint64 ip, cpu::CpuRegs& regs )
		{
			const auto targetBuf = Pointer<uint16>(regs.R3);
			const auto targetLength = (uint32)regs.R4;
			const auto writtenPtr = Pointer<uint32>(regs.R5);
			const auto sourcePtr = Pointer<char>(regs.R6);
			const auto sourceLengthBytes = (uint32)regs.R7;
		...
		{
			// input string
			const auto txt = Pointer<char>(regs.R3);
			if (txt.IsValid())
			{
				auto cur = txt;
				const auto end = txt + strlen(txt.GetNativePointer());

				// output string
				char buffer[4096];
				auto write = buffer;
				const auto writeEnd = buffer + sizeof(buffer) - 1;
					...
					if (ch == '%')
					{
						...
						switch (code)
						{
							...
							case 'S':
							{
								...
								helper::AppendString(write, writeEnd, str);
								++curReg;
								break;
							}
						}
		...
		uint64 __fastcall Xbox_RtlInitAnsiString(uint64 ip, cpu::CpuRegs& regs)
		{
			const auto ansiString = Pointer<xnative::X_ANSI_STRING>(regs.R3);
			const auto srcString = Pointer<char>(regs.R4);

			auto len = srcString.IsValid() ? strlen(srcString.GetNativePointer()) : 0;
			if (len >= 65534)
				len = 65534;
		...
		uint64 __fastcall Xbox_RtlImageXexHeaderField(uint64 ip, cpu::CpuRegs& regs)
		{
			RETURN_DEFAULT();
		}

		uint64 __fastcall Xbox_RtlCompareMemoryUlong(uint64 ip, cpu::CpuRegs& regs)
		{
			RETURN_DEFAULT();
		}

		uint64 __fastcall Xbox_RtlRaiseException(uint64 ip, cpu::CpuRegs& regs)
		{
			RETURN_DEFAULT();
		}
...
#undef MAKE_ENTRY
		...
		uint64 __fastcall Xbox_RtlNtStatusToDosError(uint64 ip, cpu::CpuRegs& regs)
		{
			auto status = (const uint32)regs.R3;
		...
		uint64 __fastcall Xbox_RtlFreeAnsiString(uint64 ip, cpu::CpuRegs& regs)
		{
			RETURN_DEFAULT();
		}

		uint64 __fastcall Xbox_RtlUnicodeStringToAnsiString(uint64 ip, cpu::CpuRegs& regs)
		{
			RETURN_DEFAULT();
		}

		uint64 __fastcall Xbox_RtlInitUnicodeString(uint64 ip, cpu::CpuRegs& regs)
		{
			RETURN_DEFAULT();
		}
