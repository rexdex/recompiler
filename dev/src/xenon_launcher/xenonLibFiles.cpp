...
namespace xenon
{
	namespace lib
	{
		...
		static uint32 CreateFileImpl(Pointer<uint32> handle_ptr, uint32 desired_access, Pointer<xnative::X_OBJECT_ATTRIBUTES> object_attributes_ptr, Pointer<xnative::X_FILE_IO_STATUS> io_status_block_ptr, Pointer<uint32> allocation_size_ptr, uint32 file_attributes, uint32 share_access, uint32 creation_disposition, uint32 creation_options)
		{
		...
		uint64 __fastcall Xbox_NtQueryInformationFile(uint64 ip, cpu::CpuRegs& regs)
		{
			...
			auto io_status_block_ptr = Pointer<xnative::X_FILE_IO_STATUS>(regs.R4);
			...
			if (file_handle != 0)
			{
				...
				if (file)
				{
					...
					else if (fileInfoType == xnative::XFileNetworkOpenInformation)
					{
						...
						{
							if (file_info_address)
								*Pointer<xnative::X_FILE_INFO>(file_info_address) = fileInfo;
		...
		uint64 __fastcall Xbox_NtReadFile(uint64 ip, cpu::CpuRegs& regs)
		{
			...
			auto io_status_block_ptr = Pointer<xnative::X_FILE_IO_STATUS>(regs.R7);
			const uint32 buffer = (const uint32)regs.R8;
			const uint32 buffer_length = (const uint32)regs.R9;
		...
		{
			auto object_attributes_ptr = Pointer<xnative::X_OBJECT_ATTRIBUTES>(regs.R3);
			auto file_info_ptr = Pointer<xnative::X_FILE_INFO>(regs.R4);
