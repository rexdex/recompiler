...
namespace crc32
{
	...
	extern const uint32 Crc32Lookup[MaxSlice][256]; // extern is needed to keep compiler happey

	/// compute CRC32 (bitwise algorithm)
	uint32 crc32_bitwise(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
		uint32 crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
		...
		while (length-- != 0)
		{
			...
			{
				// branch-free
				crc = (crc >> 1) ^ (-int32(crc & 1) & Polynomial);
	...
	uint32 crc32_halfbyte(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
		uint32 crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
		const uint8* current = (const uint8*) data;

		/// look-up table for half-byte, same as crc32Lookup[0][16*i]
		static const uint32 Crc32Lookup16[16] =
		{
	...
	uint32 crc32_1byte(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
		uint32 crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
		const uint8* current = (const uint8*) data;
	...
	uint32 crc32_4bytes(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
		uint32  crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
		const uint32* current = (const uint32*) data;

		// process four bytes at once (Slicing-by-4)
		while (length >= 4)
		{
	#if __BYTE_ORDER == __BIG_ENDIAN
			uint32 one = *current++ ^ swap(crc);
			crc = Crc32Lookup[0][ one      & 0xFF] ^
				Crc32Lookup[1][(one>> 8) & 0xFF] ^
				Crc32Lookup[2][(one>>16) & 0xFF] ^
				Crc32Lookup[3][(one>>24) & 0xFF];
	#else
			uint32 one = *current++ ^ crc;
	...
	uint32 crc32_8bytes(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
		uint32 crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
		const uint32* current = (const uint32*) data;
	...
	uint32 crc32_4x8bytes(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
		uint32 crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
		const uint32* current = (const uint32*) data;
	...
	uint32 crc32_16bytes(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
		uint32 crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
		const uint32* current = (const uint32*) data;
	...
	uint32 crc32_16bytes_prefetch(const void* data, size_t length, uint32 previousCrc32 = 0, size_t prefetchAhead = 256)
	{
	...
	uint32 crc32_fast(const void* data, size_t length, uint32 previousCrc32 = 0)
	{
	...
	const uint32 Crc32Lookup[MaxSlice][256] =
	{
...
uint32 XenonGPUCalcCRC( const void* memory, const uint32 memorySize )
{
	return crc32::crc32_16bytes_prefetch( memory, memorySize );
}

uint64 XenonGPUCalcCRC64( const void* memory, const uint32 memorySize )
{
