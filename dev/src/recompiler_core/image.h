...
namespace image
{
	...
	class RECOMPILER_API Section
	{
	public:
		inline Binary* GetImage() const { return m_image; }

		inline const std::string& GetName() const { return m_name; }
		inline const uint32 GetVirtualOffset() const { return m_virtualOffset; }
		...
		bool Load(Binary* image, IBinaryFileReader& reader);
	...
	public:
		...
		inline const uint64 GetEntryAddress() const { return m_entryAddress; }
		inline const uint64 GetTableAddress() const { return m_tableAddress; }

		Import();
		Import(Binary* parent, const uint32 index, const char* exportImageName, const uint64 tableAddress, const uint64 entryAddress, const EImportType type);
		Import(Binary* parent, const char* exportName, const char* exportImageName, const uint64 tableAddress, const uint64 entryAddress, const EImportType type);
	...
	public:
		...
		inline const uint64 GetEntryPointAddress() const { return m_entryPointAddress; }

		inline const Import* GetForwardedImport() const { return m_forwardedImport; }

		Export();
		Export(Binary* parent, const uint32 index, const char* name, const uint64 entryPointAddress);
	...
	public:
		...
		inline const uint64 GetBaseAddress() const { return m_baseAddress; }
		inline const uint64 GetEntryAddress() const { return m_entryAddress; }
	...
	public:
		...
		// save/load the object
		void Save(class ILogOutput& log, IBinaryFileWriter& writer) const;
		bool Load(class ILogOutput& log, IBinaryFileReader& reader);
		...
		const Section* FindSectionForAddress(const uint64 address) const;
