...
namespace xml
{
	...
	std::shared_ptr<Reader> Reader::LoadXML(const std::string& filePath)
	{
		// try to open file
		FILE* f = nullptr;
		...
		// create XML reader (may have parsing errors but they are to the calling site to handle)
		return std::shared_ptr<Reader>(new Reader(fileData, false));
	}

	std::shared_ptr<Reader> Reader::LoadXML(const std::wstring& filePath)
	{
		// try to open file
		FILE* f = nullptr;
