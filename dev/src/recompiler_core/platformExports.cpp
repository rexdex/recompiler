...
namespace platform
{
	...
	std::shared_ptr<ExportLibrary> ExportLibrary::LoadFromXML(ILogOutput& log, const std::wstring& filePath)
	{
		// load xml
		auto xml = xml::Reader::LoadXML(filePath);
		if (!xml.get())
			return nullptr;
		...
		std::shared_ptr<ExportLibrary> ret(new ExportLibrary());
