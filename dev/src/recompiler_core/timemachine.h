...
namespace timemachine
{
	...
	public:
		...
		static Trace* CreateTimeMachine( decoding::Context* context, const trace::Data* traceData, const uint32 traceFrameIndex );

		// get entry for given trace frame index
		const Entry* GetTraceEntry( ILogOutput& log, const uint32 traceFrameIndex );

		//---

		// get the raw reader of the trace data
		inline trace::DataReader* GetTraceReader() const { return m_traceReader; }
	...
	private:
		...
		typedef std::map< int32, Entry* >		TEntriesMap;

		decoding::Context*		m_context;

		trace::DataReader*		m_traceReader;
		Entry*					m_rootEntry;
		TEntries				m_entries;
	...
	public:
		...
		static Entry* CreateEntry( class ILogOutput& log, decoding::Context* context, Trace* trace, class trace::DataReader* reader, const int32 traceEntryIndex );
		...
		inline const uint32 GetTraceIndex() const { return m_traceIndex; }
		inline const uint32 GetCodeAddres() const {  return m_codeAddress; }
	...
	private:
		Entry( decoding::Context* context, Trace* trace, const uint32 traceIndex );
		...
		uint32						m_traceIndex;		 // decoding trace frame index
		uint32						m_codeAddress;		 // instruction address
