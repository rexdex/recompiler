...
namespace trace
{
	...
	{
		CBinaryFileChunkScope chunk( file, eFileChunk_TraceCallstackFrame );	
	...
	{
		// open file
		std::auto_ptr< IBinaryFileWriter > writer( CreateFileWriter( filePath, true ) );
		if ( !writer.get() )
			return false;
	
		// write data
		{
			CBinaryFileChunkScope chunk( *writer.get(), eFileChunk_TraceCallstack );
			*writer << m_frames;
		}
	...
	{
		// open file
		std::auto_ptr< IBinaryFileReader > reader( CreateFileReader( filePath ) );
		if ( !reader.get() )
		{
		...
		// load data
		CBinaryFileChunkScope chunk( *reader.get(), eFileChunk_TraceCallstack );
		if ( !chunk )
		{
