...
namespace decoding
{
	...
	Context::Context( const class image::Binary* module, const platform::Definition* platformDefinition )
	...
	Context* Context::Create( ILogOutput& log, const image::Binary* module, const platform::Definition* platformDefinition )
	{
		CScopedTask task( log, "Creating decoding context..." );
	...
	InstructionCache* Context::PrepareInstructionCache( const uint32 codeAddress ) const
	{
		...
		if ( m_currentSection )
		{
			const auto sectionStart = m_currentSection->GetVirtualAddress() + base;
		...
		for ( auto* cache : m_codeSectionInstructionCaches )
		{
			...
			const auto sectionStart = section->GetVirtualAddress() + base;
