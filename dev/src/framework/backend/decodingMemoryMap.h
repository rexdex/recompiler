...
namespace decoding
{
	...
	{
	public:
		inline MemoryFlags( const uint32 rva, const uint32 size, const uint32 memoryFlags, const uint32 specialFlags )
			: m_rva( rva )
			, m_size( size )
	...
	private:
		uint32		m_rva; // not saved here, just for reference
		uint8		m_size; // not saved here, just for reference
	...
	public:
		...
		inline const bool IsValidAddress( const uint32 addr ) const { return (addr>=m_baseAddress) && (addr < (m_baseAddress+m_memorySize) ); }
	...
	public:
		...
		void InvalidateRange( const uint32 rva, const uint32 size );

		// get dirty range, returns false if there's no dirty range
		bool GetDirtyRange( uint32& outFirstDirtyAddressRVA, uint32& outLastDirtyAddressRVA ) const;

		// Get memory address description, uses the RVA addressing
		const MemoryFlags GetMemoryInfo( const uint32 rva ) const;

		// Set length of block at given address
		bool SetMemoryBlockLength( ILogOutput& log, const uint32 rva, const uint32 size );

		// Set general block type of the block at specified locataion
		bool SetMemoryBlockType( ILogOutput& log, const uint32 rva, const uint32 setFlags, const uint32 clearFlags );

		// Set specialized block type of the block at specified locataion
		bool SetMemoryBlockSubType( ILogOutput& log, const uint32 rva, const uint32 setFlags, const uint32 clearFlags );
	...
	private:
		...
		uint32			m_firstDirtyAddress;
		uint32			m_lastDirtyAddress;
