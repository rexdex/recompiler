...
CFileDialog::CFileDialog( const wxString& configTag )
	: m_configTag( configTag )
{
	...
	if ( !m_configTag.empty() )
	{
		...
		if ( dir.length() )
		{
			setDirectory( dir.c_str().AsWChar() );
		}*/
	}
}
...
void CFileDialog::ClearFormats()
{
	m_formats.clear();
}

void CFileDialog::AddFormat( const wxString& extension, const wxString& description )
{
	// Make sure format is not already defined
	for ( size_t i=0; i<m_formats.size(); ++i )
	{
		const CFileFormat& desc = m_formats[i];
	...
	// Add new format
	m_formats.push_back( CFileFormat( extension, description ) );
}

void CFileDialog::AddFormat( const CFileFormat& format )
{
	// Make sure format is not already defined
	for ( size_t i=0; i<m_formats.size(); ++i )
	{
...
void CFileDialog::SetMultiselection( bool multiselection )
{
	if ( multiselection )
	{
...
void CFileDialog::SetDirectory( const wxString& directory )
{
	wcscpy_s( (wchar_t*)m_settings.lpstrInitialDir, _MAX_PATH, directory.wc_str() );
}

bool CFileDialog::DoOpen( wxWindow* owner, const wxString& defaultFileName /*= wxEmptyString*/ )
{
...
bool CFileDialog::DoSave( wxWindow* owner, const wxString& defaultFileName /*= wxEmptyString*/ )
{
	// Remember current directory
	wchar_t tempDir[ _MAX_PATH ];
	GetCurrentDirectoryW( _MAX_PATH, tempDir );
...
void CFileDialog::UpdateFilters( bool all )
{
