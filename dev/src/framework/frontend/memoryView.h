...
public:
	...
	virtual uint32 GetAddressInfo( const uint32 address, uint32& outNumLines, uint32& outNumBytes ) const = 0;

	//! Generate the text representation for, only valid addresses should be accepted
	virtual bool GetAddressText( const uint32 address, IMemoryLinePrinter& printer ) const = 0;

	//! Get address hit count (if any)
	virtual uint32 GetAddressHitCount( const uint32 address ) const { return 0; }

	//! Get any special markers to show on the line
	virtual bool GetAddressMarkers( const uint32 address, uint32& outMarkers, uint32& outLineOffset ) const { return false; }

	//! Show context memory for given line range
	virtual bool ShowContextMenu( class CMemoryView* view, const uint32 startOffset, const uint32 endOffset, const wxPoint& point ) { return false; }

	//! Activate selection (enter key)
	virtual bool Navigate( class CMemoryView* view, const uint32 startOffset, const uint32 endOffset, const bool bShift ) { return false; }

	//! Navigate back
	virtual bool NavigateBack( class CMemoryView* view ) { return false; }
...
/// Memory address info (for nice dispaly)
class CMemoryLineEntry
{
private:
	// text buffer (managed localy)
	...
	//   anything inside the [] is an memory adress, only the numerical value is considered
	char*			m_text;
...
private:
	...
	friend class CMemoryLineAllocator;
...
private:
	...
	struct Block
	{
		CMemoryLineEntry*	m_base;
		CMemoryLineEntry*	m_end;
		uint32				m_count;
		uint32				m_max;

		Block( void* base, uint32 count )
			: m_base( (CMemoryLineEntry*)base )
			, m_end( m_base + count )
			, m_max( count )
			, m_count( 0 )
		{}
	...
	CMemoryLineEntry*		m_freeList;
	Block*					m_freeBlock;

	std::vector< Block* >	m_blocks;

public:
	CMemoryLineAllocator();
	~CMemoryLineAllocator();

	//! Allocate line entry
	CMemoryLineEntry* AllocateLineEntry();

	//! Free line entry
	void FreeLineEntry( CMemoryLineEntry* lineEntry );
...
class CMemoryLineCache : private IMemoryLinePrinter
{
public:
	// cache entry
	struct Entry
	{
		CMemoryLineEntry*	m_line;
		Entry*				m_next;		// if we have more lines

		inline Entry( CMemoryLineEntry* line, Entry*& listHead, Entry*& listTail )
			: m_line( line )
			, m_next( nullptr )
		{
			if ( !listHead )
		...
		inline void Free( class CMemoryLineAllocator& lineAlloc )
		{
			if ( nullptr != m_line )
			{
...
private:
	// line allocator 
	class CMemoryLineAllocator*			m_lineAllocator;
...
public:
	CMemoryLineCache( class CMemoryLineAllocator* allocator );
	~CMemoryLineCache();
...
class CMemoryView : public wxWindow, public ISyncUpdate
{
	DECLARE_EVENT_TABLE();
...
protected:
	...
	//! Line allocator
	CMemoryLineAllocator		m_lineAllocator;

	//! Memory <-> Line mapping
	CLineMemoryMapping			m_lineMapping;

	//! Cache for lines
	CMemoryLineCache			m_lineCache;
...
public:
	...
	bool GetSelectionAddresses( uint32& startAddress, uint32& endAddress ) const;

	//! Get the RVA for cursor position (selection end)
	uint32 GetCurrentRVA() const;
