...
CTimeMachineView::SlotInfo::SlotInfo( timemachine::Trace* trace, LayoutInfo* node, const timemachine::Entry::AbstractDependency* dependency )
...
CTimeMachineView::SlotInfo::SlotInfo( timemachine::Trace* trace, LayoutInfo* node, const timemachine::Entry::AbstractSource* source )
...
CTimeMachineView::SlotInfo::~SlotInfo()
{
}

//---------------------------------------------------------------------------

CTimeMachineView::LayoutInfo::LayoutInfo( timemachine::Trace* trace, const timemachine::Entry* entry )
	: m_position(0,0)
	, m_size( 100, 30 )
	, m_entry( entry )
...
CTimeMachineView::LayoutInfo::~LayoutInfo()
{
...
CTimeMachineView::CTimeMachineView( wxWindow* parent, class timemachine::Trace* trace )
	: Canvas::CCanvasPanel( parent, wxCLIP_CHILDREN | wxCLIP_SIBLINGS | wxNO_BORDER )
...
CTimeMachineView::~CTimeMachineView()
{
	// delete visual nodes
	ClearNodes();
...
const int32 CTimeMachineView::GetRootTraceIndex() const
{
	return m_trace->GetRoot()->GetTraceIndex();
}

void CTimeMachineView::ShowRootNode()
{
	m_requestedLayoutChange = 1;
}

void CTimeMachineView::ShowAllNodes()
{
	m_requestedLayoutChange = 2;
}

void CTimeMachineView::DoShowRootNode( const int w, const int h )
{
	// no nodes
	if ( m_nodes.empty() )
		return;
	...
	// redraw
	CCanvasPanel::Repaint();
}

void CTimeMachineView::DoShowAllNodes( const int w, const int h )
{
	// no nodes
	if ( m_nodes.empty() )
		return;
...
void CTimeMachineView::ClearNodes()
{
	// reset trace
	m_trace = nullptr;
...
void CTimeMachineView::DeselectAll()
{
	m_selection.clear();
	Refresh();
}

void CTimeMachineView::Select( const timemachine::Entry* entry, const bool isSelected )
{
...
const bool CTimeMachineView::IsSelected( const timemachine::Entry* entry ) const
{
	return std::find( m_selection.begin(), m_selection.end(), entry ) != m_selection.end();
}

const timemachine::Entry* CTimeMachineView::GetEntryAtPos( const wxPoint mousePos )
{
	PrepareLayout();
...
CTimeMachineView::SlotInfo* CTimeMachineView::GetUnconnectedSlotAtPos( const wxPoint mousePos )
{
	PrepareLayout();
...
void CTimeMachineView::ExploreSlot( SlotInfo* slot )
{
	// resolve the dependencies
	if ( slot && slot->m_abstractDep )
	{
		// show the progress bar
		CScopedTask task( *wxTheFrame->GetLogView(), "Resolving history" );
		wxTheFrame->GetLogView()->SetTaskName("Looking for dependencies...");
		
		// resolve the sources
		std::vector< const timemachine::Entry::AbstractSource* > resolvedSources;
		slot->m_abstractDep->Resolve( *wxTheFrame->GetLogView(), m_trace, resolvedSources );
...
CTimeMachineView::LayoutInfo* CTimeMachineView::GetLayoutNode( const timemachine::Entry* entry )
{
	// invalid entry
	if ( !entry )
		return nullptr;
...
CTimeMachineView::SlotInfo* CTimeMachineView::GetLayoutSlot( const timemachine::Entry::AbstractSource* source )
{
	// invalid source
	if ( !source )
		return nullptr;
...
CTimeMachineView::SlotInfo* CTimeMachineView::GetLayoutSlot( const timemachine::Entry::AbstractDependency* dep )
{
	// invalid source
	if ( !dep )
		return nullptr;
...
void CTimeMachineView::PrepareNodePlacement()
{
...
void CTimeMachineView::PrepareLayout()
{
	// get the dirty nodes
	...
	for ( auto it : nodes )
	{
		...
		const wxSize textSize = Canvas::CCanvasPanel::GetTextExtent( it->m_caption );
		...
		{
			const wxSize textSize = Canvas::CCanvasPanel::GetTextExtent( jt->m_name );
		...
		// place the input slots
		{
			...
			{
				const wxSize textSize = Canvas::CCanvasPanel::GetTextExtent( it->m_inputs[i]->m_name );
		...
		// place the output slots
		{
			...
			{
				const wxSize textSize = Canvas::CCanvasPanel::GetTextExtent( it->m_outputs[i]->m_name );
...
void CTimeMachineView::OnPaintCanvas( int width, int height )
{
	// background
...
void CTimeMachineView::OnMouseClick( wxMouseEvent& event )
{
	if ( m_mouseMode == 0 )
	{
		if ( event.LeftDown() )
...
void CTimeMachineView::OnMouseMove( wxMouseEvent& event, wxPoint delta )
{
	if ( m_mouseMode == 1 )
	{
...
void CTimeMachineView::UpdateHover( const wxPoint& mousePoint )
{
	bool refresh = false;
...
void CTimeMachineView::OnMouseTrack( wxMouseEvent& event )
{
