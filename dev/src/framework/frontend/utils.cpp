...
namespace Parse
{
	...
	void PrintCallHistory( class ILogOutput& log, const uint32 level, const ProjectTraceCallHistory& history )
	{
		if ( level > 255 )
			return;

		char lead[256];
		memset( lead, 32, level );
		lead[level] = 0;

		log.Log( "CallStack: %u %sEnter %s %08Xh, TR=#%05u", level, lead,
			history.m_functionName.c_str(), history.m_functionStart, history.m_traceEnter );

		for ( uint32 i=0; i<history.m_childCalls.size(); ++i )
		{
			PrintCallHistory( log, level+1, history.m_childCalls[i] );
		}

		if ( history.m_traceLeave )
		{
			log.Log( "CallStack: %u %sLeave %s %08Xh, TR=#%05u", level, lead,
				history.m_functionName.c_str(), history.m_functionStart, history.m_traceLeave );
		}
	}

	void PrintValueHistory( class ILogOutput& log, const std::vector< ProjectTraceAddressHistoryInfo >& history )
	{
		log.Log( "History: Code location visited %u times", (uint32)history.size() );
		for ( uint32 i=0; i<history.size(); ++i )
		{
			const ProjectTraceAddressHistoryInfo& info = history[i];

			log.Log( "History: trace at #%06u, value of '%hs' = %hs", 
				info.m_traceEntry, info.m_register->GetName(), info.m_value.c_str() );
		}
	}
