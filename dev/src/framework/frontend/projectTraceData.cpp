...
#include "..\backend\traceMemoryHistory.h"
#include "..\backend\timemachine.h"
...
#include "..\backend\traceMemoryHistoryBuilder.h"
...
{
	return timemachine::Trace::CreateTimeMachine( m_project->GetEnv().GetDecodingContext(), m_data, traceFrameIndex );
}
...
bool ProjectTraceData::MoveToEntry(const uint32 entryIndex)
{
	if (entryIndex < m_data->GetNumDataFrames())
	{
		if (entryIndex != m_currentEntry)
		{
			...
			wxTheEvents->EventTraceIndexChanged( entryIndex );
		}
...
{
	// memory map
	const auto& memMap = m_project->GetEnv().GetDecodingContext()->GetMemoryMap();
...
{
	CScopedTask task(*wxTheFrame->GetLogView(),"Scanning...");
	wxTheFrame->GetLogView()->SetTaskName("Scanning for memory access...");
	...
	auto* context = m_project->GetEnv().GetDecodingContext();
	while (entryIndex > 0)
	{
		// stats		
		wxTheFrame->GetLogView()->SetTaskProgress(startEntry-entryIndex, startEntry);
		...
		{
			{
				...
				wxTheFrame->GetLogView()->Log( "Trace: Instruction accessing memory: '%s'", commandName );
			}
...
bool ProjectTraceData::FindFirstMemoryAccess(const uint32 startEntry, const uint32 memoryAddress, const uint32 size, const uint8 actionMask, uint32& outEntryIndex, uint32& outCodeAddress) const
{
	...
	{
		// stats		
		wxTheFrame->GetLogView()->SetTaskProgress(entryIndex, startEntry);
		...
		{
			{
				...
				char* stream = commandName;
				op.GenerateText(codeAddress, stream, stream + sizeof(commandName));
				wxTheFrame->GetLogView()->Log( "Trace: First instruction accessing memory: '%s'", commandName );
			}
...
bool ProjectTraceData::FindAllMemoryAccess(const uint32 memoryAddress, const uint32 size, const uint8 actionMask, std::vector< ProjectTraceMemoryAccessEntry >& outEntries ) const
{
	...
	{
		// stats		
		wxTheFrame->GetLogView()->SetTaskProgress(entryIndex, lastEntry);
...
bool ProjectTraceData::FindForwardMemoryAccess(const uint32 startEntry, const uint32 memoryAddress, const uint32 size, const uint8 actionMask, uint32& outEntryIndex, uint32& outCodeAddress) const
{
	...
	uint32 entryIndex = startEntry + 1;
	const uint32 lastEntry = m_data->GetNumDataFrames();
	auto* context = wxTheFrame->GetProject()->GetEnv().GetDecodingContext();
	while (entryIndex < lastEntry)
	{
		// stats		
		wxTheFrame->GetLogView()->SetTaskProgress(entryIndex-startEntry, lastEntry-startEntry);
...
bool ProjectTraceData::FindForwardRegisterAccess(const uint32 startEntry, const char* regName, const uint8 actionMask, uint32& outEntryIndex, uint32& outCodeAddress) const
{
	...
	if ( !reg )
	{
		wxTheFrame->GetLogView()->Log( "No register '%s'!", regName );
		return false;
	}
		...
		{
			{
				...
				char* stream = commandName;
				op.GenerateText(codeAddress, stream, stream + sizeof(commandName));
				wxTheFrame->GetLogView()->Log( "Trace: Instruction accessing register: '%s'", commandName );
			}
...
bool ProjectTraceData::BuildMemoryMap()
{
	...
	if (!builder.BuildFile( *wxTheFrame->GetLogView(), *m_project->GetEnv().GetDecodingContext(), *m_data, memTracePath.c_str() ) )
		return false;

	// open the memory file	
	m_memory = trace::MemoryHistory::OpenFile(*wxTheFrame->GetLogView(), memTracePath.c_str());
	if (!m_memory)
		return false;
...
bool ProjectTraceData::BuildCallstack(ILogOutput& log)
{
	...
	// progress bar
	struct ProgressDisplay : public trace::CallTree::IStackBuilderProgress
	{
	public:
		ProgressDisplay()
		{
			m_lastPrc = 0;
			CProgressDialog::GetInstance().StartTask( "Building callstack" );
			CProgressDialog::GetInstance().UpdateTaskProgress( 0 );
		}
		...
		virtual void OnProgress( const uint32 position, const uint32 total ) override
		{
			uint32 prc = (uint32) ((uint64)position * 1000) / (uint64)total;
			if ( prc != m_lastPrc )
			{
				m_lastPrc = prc;
				CProgressDialog::GetInstance().UpdateTaskProgress( prc/10 );
				CProgressDialog::GetInstance().UpdateTaskTextf( "Analyzing trace data (%d/%d)...", position, total );
			}
		}
	...
	// try to build the memory trace file
	trace::CallTree* callstack = trace::CallTree::BuildTraceCalltack( log, *wxTheFrame->GetProject()->GetEnv().GetDecodingContext(), *m_data, 0, progressDisplay );
	if ( !callstack )
	{
...
{
	CScopedTask task(*wxTheFrame->GetLogView(),"Scanning...");
	wxTheFrame->GetLogView()->SetTaskName("Scanning for register values...");
	...
	if ( !reg )
	{
		wxTheFrame->GetLogView()->Error( "No register '%s' in cpu '%s'!", regName, cpu->GetName().c_str() );
		return false;
	}
	...
	{
		// stats		
		wxTheFrame->GetLogView()->SetTaskProgress(entryIndex-startTrace, endEntryIndex-startTrace);
...
{
	CScopedTask task(log, "Loading trace...");

	// load raw data
	trace::Data* data = trace::Data::OpenTraceFile( *wxTheFrame->GetLogView(), filePath );
	if ( !data ) 
		return NULL;
