...
CMemoryLineEntry::CMemoryLineEntry()
	: m_text( nullptr )
{
}
...
void CMemoryLineEntry::Set( const char* textToCopy )
{
...
CMemoryLineAllocator::CMemoryLineAllocator()
	: m_freeList( nullptr )
	, m_freeBlock( nullptr )
{
	AllocateNewBlock();
...
void CMemoryLineAllocator::AllocateNewBlock()
{
	char* mem = new char [ BLOCK_SIZE ];
	const uint32 numLinesPerBlock = BLOCK_SIZE / sizeof(CMemoryLineEntry);
	m_freeBlock = new Block( mem, numLinesPerBlock );
	m_blocks.push_back( m_freeBlock );
}
 
CMemoryLineEntry* CMemoryLineAllocator::AllocateLineEntry()
{
	// use the free list if possible
	if ( m_freeList != nullptr )
	{
		CMemoryLineEntry* entry = m_freeList;
		m_freeList = (CMemoryLineEntry*) m_freeList->m_text;
		return entry;
	}
	...
	CMemoryLineEntry* entry = m_freeBlock->m_base + m_freeBlock->m_count;
	m_freeBlock->m_count += 1;
	return entry;
}

void CMemoryLineAllocator::FreeLineEntry( CMemoryLineEntry* lineEntry )
{
	// free internals
	lineEntry->Reset();
...
CMemoryLineCache::CMemoryLineCache( CMemoryLineAllocator* allocator )
	: m_lineAllocator( allocator )
{
}

CMemoryLineCache::~CMemoryLineCache()
{
}

void CMemoryLineCache::Clear()
{
	// cleanup the map
...
void CMemoryLineCache::ClearRange( const uint32 firstAddress, const uint32 lastAddress )
{
...
const CMemoryLineCache::Entry* CMemoryLineCache::GetCachedLines( const uint32 offset, IMemoryDataView& dataView )
{
...
void CMemoryLineCache::Print( const char* text, ... )
{
	char buffer[ 4096 ];
	va_list args;
	...
	// create the line
	CMemoryLineEntry* line = m_lineAllocator->AllocateLineEntry();
	if ( nullptr != line )
	{
		// set the line text
		line->Set( buffer );
...
BEGIN_EVENT_TABLE( CMemoryView, wxWindow )
	EVT_LEFT_DOWN( CMemoryView::OnMouseDown )
	EVT_LEFT_DCLICK( CMemoryView::OnMouseDown )
	EVT_RIGHT_DOWN( CMemoryView::OnMouseDown )
	EVT_LEFT_UP( CMemoryView::OnMouseUp )
	EVT_RIGHT_UP( CMemoryView::OnMouseUp )
	EVT_MOTION( CMemoryView::OnMouseMove )
	EVT_MOUSEWHEEL( CMemoryView::OnMouseWheel )
	EVT_MOUSE_CAPTURE_LOST( CMemoryView::OnCaptureLost )
	EVT_PAINT( CMemoryView::OnPaint )
	EVT_ERASE_BACKGROUND( CMemoryView::OnErase )
	EVT_KEY_DOWN( CMemoryView::OnKeyDown )
	EVT_CHAR_HOOK( CMemoryView::OnCharHook )
	EVT_SCROLLWIN( CMemoryView::OnScroll )
	EVT_SIZE( CMemoryView::OnSize )
END_EVENT_TABLE()

CMemoryView::DrawingStyle::DrawingStyle()
	: m_lineHeight( 18 )
	, m_lineSeparation( 0 )
	, m_addressOffset( 20 )
	, m_hitcountSize( 40 )
...
CMemoryView::CMemoryView( wxWindow* parent )
...
void CMemoryView::ClearCachedData()
{
	m_lineCache.Clear();
}

uint32 CMemoryView::GetNumLines() const
{
	return m_lineMapping.GetNumberOfLines();
}

bool CMemoryView::GetSelectionOffsets( uint32& startOffset, uint32& endOffset ) const
{
	if ( m_view != nullptr )
	{
		// get selection lines
...
bool CMemoryView::GetSelectionAddresses( uint32& startAddress, uint32& endAddress ) const
{
	if ( m_view != nullptr )
	{
...
uint32 CMemoryView::GetCurrentRVA() const
{
	if ( m_view != nullptr )
	{
		uint32 startAddrses=0, endAddress=0;
...
void CMemoryView::SetDataView( IMemoryDataView* view )
{
	// different ?
	if ( view != m_view ) 
	{
...
bool CMemoryView::SetHighlightedOffset( const uint32 offset )
{
	m_highlightedOffset = offset;
	return true;
}

bool CMemoryView::SetActiveOffset( const uint32 offset, bool createHistoryEntry /*= true*/ )
{
	bool status = false;
	...
	if ( m_view != nullptr )
	{
		...
		{
			selectionDisplacement = numLinesOnScreen / 2; // show in the middle of the screen
		}
...
void CMemoryView::SetFirstVisibleLine( int line )
{
...
void CMemoryView::EnsureVisible( int line )
{
	const int numLinesOnScreen = (GetClientSize().y-(m_style.m_lineHeight-1)) / m_style.m_lineHeight;
...
void CMemoryView::SetSelectionCursor( int lineIndex, const bool selectionEnd )
{
...
void CMemoryView::MoveSelectionCursor( int numLines, const bool selectionEnd )
{
	const int numTotalLines = m_lineMapping.GetNumberOfLines();
...
void CMemoryView::NavigateSelection()
{
	if ( nullptr != m_view )
	{
...
void CMemoryView::UpdateScrollbar()
{
...
int CMemoryView::GetLineAtPixel( const wxPoint& point ) const
{
	// size of the DC
...
void CMemoryView::SetHitcountDisplay( const bool enabled )
{
...
void CMemoryView::RefreshDirtyMemoryRegion()
{
...
void CMemoryView::RefreshMemoryRegion( const uint32 startOffset, const uint32 endOffset )
{
	if ( m_view != nullptr )
	{
...
void CMemoryView::RefreshMemoryRegionRVA( const uint32 startRVA, const uint32 endRVA )
{
}
...
void CMemoryView::OnMouseUp( wxMouseEvent& event )
{
...
void CMemoryView::OnMouseWheel( wxMouseEvent& event )
{
	int numLines = 3;
...
void CMemoryView::OnPaint( wxPaintEvent& event )
{
	...
	while ( lineY <= size.y && lineIndex < numMappedLines)
	{
		...
		const CMemoryLineCache::Entry* curLine = m_lineCache.GetCachedLines( memoryOffset, *m_view );
		if ( curLine == nullptr )
		{
			// skip this line
			lineY += m_style.m_lineHeight;
...
void CMemoryView::OnCharHook( wxKeyEvent& event )
{
...
void CMemoryView::OnKeyDown( wxKeyEvent& event )
{
	const bool selectionMode = wxGetKeyState( WXK_SHIFT );
...
void CMemoryView::OnScroll( wxScrollWinEvent& event )
{
	SetFirstVisibleLine( event.GetPosition() );
}

void CMemoryView::OnSize( wxSizeEvent& event )
{
	SetFirstVisibleLine( m_firstVisibleLine );
	event.Skip();
}

void CMemoryView::OnCaptureLost( wxMouseCaptureLostEvent& event )
{
	m_selectionDragMode = false;
	Refresh();
}

void CMemoryView::OnSyncUpdate()
{
